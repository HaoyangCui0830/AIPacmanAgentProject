...
class ReflexCaptureAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if enemyNearby:
        ...
        for cell in grid:
            ...
            if self.available_actions[cell] == 1 and cell not in foodList:
                rewardTable.rewardDistribution(cell, potentialDeadEndReward * 2)
    ...
    if (len(foodList) <= 2) or (timeLeft < 40) or (timeLeft < (self.getDistanceHome(currentPosition) + 10)):
        for i in self.homeBoundaryCells:
...
class AstarAgent(ReflexCaptureAgent):
    ...
    def getWeights(self, gameState, action):
        return {'num_of_pacman_enemies': -1000, 'distanceHome':-2, 'defending': 100, 'closest_enemy_dist': -10, 'stop': -100, 'reverse': -2}
